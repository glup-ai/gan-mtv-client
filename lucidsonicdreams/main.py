...
class LucidSonicDream:
    ...
    def hallucinate(
        ...
        try:
            joblib.dump(self.noise, "noise.txt")
            joblib.dump(self.class_vecs, "class_vecs.txt")
        except:
            pass
        try:
            out_file = open("noise.json", "w")
            json.dump(self.noise, out_file, indent=6)
            out_file.close()
            out_file = open("class_vecs.json", "w")
            json.dump(self.class_vecs, out_file, indent=6)
            out_file.close()
        except:
            pass
